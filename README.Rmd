...
and the method to do this is [`rq_table()`](https://winvector.github.io/rquery/reference/rq_table.html),
which builds a reference to a remote table given the table name.  The suggested pattern for working with 
remote tables is to get inputs via [`rq_table()`](https://winvector.github.io/rquery/reference/rq_table.html)
...
We also could stand `rquery` up on non-`DBI` sources such as [`SparkR`](https://github.com/WinVector/rquery/blob/master/extras/SparkRExample.md)
