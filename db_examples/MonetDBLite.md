...
Now we re-write the original calculation in terms of the `rquery` SQL generating operators. `MMonetDBLite` doesn't currenlty declare `sum()` as a window function, so we have to normalize probabilities with a join.
...
```
    ...
    ## [1] "table(\"rquery_mat_80915920707218448994_0000000000\"; subjectID, diagnosis, probability)"
      ...
      FROM (
       SELECT * FROM (
        ...
        FROM (
         ...
         FROM (
          SELECT
           COALESCE("tsql_66226500961600370688_0000000003"."subjectID", "tsql_66226500961600370688_0000000004"."subjectID") AS "subjectID",
           "tsql_66226500961600370688_0000000003"."surveyCategory" AS "surveyCategory",
           "tsql_66226500961600370688_0000000003"."probability" AS "probability",
           "tsql_66226500961600370688_0000000004"."tot_prob" AS "tot_prob"
          FROM (
           SELECT
            "subjectID",
            "surveyCategory",
           ...
           FROM (
            ...
            FROM
             "d" LIMIT 1000
            ) tsql_66226500961600370688_0000000000
          ) "tsql_66226500961600370688_0000000003"
          LEFT JOIN (
           SELECT "subjectID", sum ( "probability" ) AS "tot_prob" FROM (
            ...
            FROM (
             ...
             FROM
              "d" LIMIT 1000
             ) tsql_66226500961600370688_0000000001
            ) tsql_66226500961600370688_0000000002
           GROUP BY
            "subjectID"
          ) "tsql_66226500961600370688_0000000004"
          ON
           "tsql_66226500961600370688_0000000003"."subjectID" = "tsql_66226500961600370688_0000000004"."subjectID"
          ) tsql_66226500961600370688_0000000005
         ) tsql_66226500961600370688_0000000006
       ) tsql_66226500961600370688_0000000007
       WHERE "row_number" <= 1
      ) tsql_66226500961600370688_0000000008
     ) tsql_66226500961600370688_0000000009
    ) tsql_66226500961600370688_0000000010 ORDER BY "subjectID"
