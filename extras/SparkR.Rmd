...
We can do this in R using `dplyr`, another data manipulation package with a Codd-style notation:
...
iris %>%
  ...
  mutate(rank = dense_rank(-mean_petal_width)) %>%
  filter(rank == 1) %>%
  select(Species)
...
In `dplyr`, the original table (`iris`) is at the beginning of the query, with successive operations applied to the results of the preceding line.  To perform the equivalent operation in SQL, you must write down the operation "backwards":
...
Like `dplyr`, `rquery` represents an attempt to make data transformation in a relational database more intuitive by expressing data transformations in a sequential operator pipeline notation instead of nested queries or functions. 

## `rquery` vs `dplyr`

For developers working with Spark and R, `rquery` offers a number of advantages over `dplyr`. In particular, `dplyr` is not compatible with `SparkR`, only with `sparklyr`. In addition, `rquery` is *database-first*, meaning it was developed specifically to work with big data in remote data stores via R.


For example, unlike `dplyr`, `rquery` does not rely on persistent table annotations such as the `group by` annotation. 

problems that these address:
you don't want to run a long query only to find out the last step is wrong

complete separation between query specification and execution phases
aggressive column name error checking
query narrowing (early column filtering)
higher order operators, yielding shorter pipelines



## Design Choices for SQL implementation

Stay close to Cod's definitions. In contrast to other R data manipulation packages, rquery does not rely on db-unsafe annotations such  or "group by" annotations. Instead, grouped aggregations are implemented via window functions in SQL. 
...
R developers can run analyses and perform data transformations in Spark using an easier to read (and to write) sequential pipeline notation instead
...
You can also inspect the (complex and heavily nested) SQL query that will be executed in the cluster. Notice that the columb `orderID`, which is not involved in this query, is already eliminated in the initial SELECT (tsql_*_0000000000). Winnowing the initial tables down to only the columns used can be a big performance improvement when you are working with excessively wide tables, and using only a few columns.
...
Many R programmers are familiar with `dplyr`, another data manipulation package in R, which also uses Codd-style operators. We can compare `dplyr`'s operators to those of `rquery` (and verify that the calculation is correct), by processing a local copy of the data with `dplyr`. 
