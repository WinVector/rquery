...
We can build non-trivial operations into `sql_node()`s.  For example 
...
sql <- to_sql(op_tree_count_null, my_db)
cat(sql)

# execute
execute(my_db, op_tree_count_null)
...
count on round-tripping through `R`).  Another important `SQL`-first package is [`cdata`](https://github.com/WinVector/cdata/)
which provides pure `SQL` based implementations of operators that easily generalize pivot/un-pivot, cast/melt, or spread/gather.
...
The downside of `SQL` first, is some operators require using the `sql_node()` as
we are not yet translating some `R` expression idiom to `SQL`.
...
Another higher-order node is the [`if_else_block()`](https://winvector.github.io/rquery/reference/if_else_block.html) which can
...
and database based `dplyr` (article [here](http://www.win-vector.com/blog/2018/01/rquery-fast-data-manipulation-in-r/)).
...
has show significant additional performance advantages (article [here](http://www.win-vector.com/blog/2017/12/how-to-greatly-speed-up-your-spark-queries/)).
...
For a project primarily interested in working a big data scale (say using `PostgreSQL` or `Spark`)
`rquery` is the correct specialized tool for specifying data manipulation.
