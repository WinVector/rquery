...
[`rquery`](https://winvector.github.io/rquery/) is a query generator
...
production). One goal of this experiment is to see if `SQL` would be
more fun teachable if it had a sequential data-flow or pipe notation.

`rquery` is currently experimental, and not yet recommended for
production use.
...
advanced `SQL` training tool (it shows how some very deep `SQL` by
composing `rquery` operators). Currently `rquery` is biased towards the
`Spark` and `PostgeSQL` `SQL` dialects.
...
The primary relational operators
  ...
  - [`extend()`](https://winvector.github.io/rquery/reference/extend_nse.html).
  ...
  - [`project()`](https://winvector.github.io/rquery/reference/project_nse.html).
  ...
  - [`natural_join()`](https://winvector.github.io/rquery/reference/natural_join.html).
    ...
    condition.
  - [`theta_join()`](https://winvector.github.io/rquery/reference/theta_join_nse.html).
    This is the relational join operator allowing an arbitrary matching
    predicate.
  - [`select_rows()`](https://winvector.github.io/rquery/reference/theta_join_nse.html).
  ...
  - [`rename_columns()`](https://winvector.github.io/rquery/reference/rename_columns.html).
    This operator renames sets of columns.
...
The primary non-relational (traditional `SQL`) operators
  ...
  - [`orderby()`](https://winvector.github.io/rquery/reference/orderby.html).
    ...
    row-order is usually maintained).
...
The actual `SQL` query that produces the result is, in fact, quite
involved:
       ...
       FROM (
        ...
        FROM (
         ...
         FROM (
          ...
          FROM
           `d` LIMIT 1000
          ) tsql_14284904044118466261_0000000000
         ) tsql_14284904044118466261_0000000001
       ) tsql_14284904044118466261_0000000002
      ) tsql_14284904044118466261_0000000003
      WHERE `rank` = `count`
     ) tsql_14284904044118466261_0000000004
    ) tsql_14284904044118466261_0000000005 ORDER BY `subjectID`
...
experiments](https://github.com/WinVector/rquery/blob/master/extras/PerfTest.md)
...
Part of the plan is: the additional record-keeping in the operator nodes
would let a potentially powerful query optimizer work over the flow
before it gets translated to `SQL` (perhaps an extension of or successor
to [`seplyr`](https://winvector.github.io/seplyr/), which re-plans over
`dplyr::mutate()` expressions). At the very least restricting to columns
later used and folding selects together would be achievable. One should
have a good chance at optimization as the representation is fairly
high-level, and many of the operators are relational (meaning there are
known legal transforms a query optimizer can use). The flow itself is
...
We also can stand `rquery` up on non-`DBI` sources such as
[`SparkR`](https://github.com/WinVector/rquery/blob/master/extras/SparkRExample.md)
and perhaps even
[`data.table`](https://github.com/WinVector/rquery/blob/master/extras/data_table.md).
