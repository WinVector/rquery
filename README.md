...
`rquery` is currently recommended for user with `Spark` and `PostgreSQL`
(and with non-window functionality with `RSQLite`).

To install: `devtools::install_github("WinVector/rquery")` or
`install.packages("rquery")`.

A good place to start is the [`rquery` introductory
vignette](https://winvector.github.io/rquery/articles/rquery_intro.html).
...
The primary non-relational (traditional `SQL`) operators
  ...
  - [`map_column_values()`](https://winvector.github.io/rquery/reference/map_column_values.html)
    re-map values in columns (very useful for re-coding
    data).
...
[`rq_table()`](https://winvector.github.io/rquery/reference/rq_table.html),
        ...
        FROM (
         ...
         FROM (
          ...
          FROM (
           ...
           FROM
            "d" LIMIT 1000
           ) tsql_73312825821989384495_0000000000
          ) tsql_73312825821989384495_0000000001
         ) tsql_73312825821989384495_0000000002
       ) tsql_73312825821989384495_0000000003
       WHERE "row_number" <= 1
      ) tsql_73312825821989384495_0000000004
     ) tsql_73312825821989384495_0000000005
    ) tsql_73312825821989384495_0000000006 ORDER BY "subjectID"
...
[here](https://github.com/WinVector/rquery/blob/master/extras/PerfTest.md)).

We also could stand `rquery` up on non-`DBI` sources such as
...
For deeper dives into specific topics, please see also:
  ...
  - [Join
    Controller](https://github.com/WinVector/rquery/blob/master/extras/JoinController.md)
  - [Join Dependency
  ...
  - [Assignment
    Partitioner](https://github.com/WinVector/rquery/blob/master/extras/AssigmentPartitioner.md)
